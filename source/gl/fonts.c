...
void DrawText(const char* text, float x, float y, float scale, Color color) {
    ...
    const float lineHeight = charHeight * 1.0f;
    ...
    for (const char* c = text; *c; c++) {
        if (*c == '\n') {
            glPopMatrix();
            glPushMatrix();
            glTranslatef(x, y - currentY - lineHeight, 0.0f);
            glScalef(scale, scale, 1.0f);
            currentY -= lineHeight;
            startX = 0.0f;
            continue;
        ...
        const unsigned char* bitmap = font6x8_ascii[(int)*c];
        
        for (int row = 0; row < charHeight; row++) {
            for (int col = 0; col < charWidth; col++) {
                if (bitmap[col] & (1 << row)) {
                    glBegin(GL_QUADS);
                        glVertex2f(startX + col, currentY - row);
                        glVertex2f(startX + col + 1, currentY - row);
                        glVertex2f(startX + col + 1, currentY - row + 1);
                        glVertex2f(startX + col, currentY - row + 1);
                    glEnd();
                }
            }
        }
